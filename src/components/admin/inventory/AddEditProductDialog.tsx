...
const productSchema = z.object({
  ...
  minStock: z.preprocess(
    (val) => val === '' || val === null || val === undefined ? undefined : parseInt(String(val), 10), // Handle empty string/null for optional
    z.number().int().min(0, { message: 'El stock mínimo no puede ser negativo.' }).optional() // Make minStock optional
  ),
  margin: z.preprocess(
    (val) => val === '' || val === null || val === undefined ? undefined : parseFloat(String(val).replace(/[^0-9.]+/g, "")), // Handle empty string/null for optional
...
const AddEditProductDialog: React.FC<AddEditProductDialogProps> = ({
  ...
  const form = useForm<ProductFormValues>({
    ...
    defaultValues: {
      ...
      minStock: undefined, // Initialize minStock as undefined
    ...
    if (isOpen) {
      if (isEditMode && product) {
        ...
        form.reset({
          ...
          minStock: product.minStock ?? undefined, // Reset minStock in edit mode
        ...
        form.reset({
          ...
          minStock: undefined, // Default undefined for minimal add
        ...
        form.reset({
          ...
          minStock: undefined, // Default undefined
          margin: undefined,
        });
      }
  ...
  const mutationFn = async (values: ProductFormValues): Promise<Product> => { // Return added/updated product
    ...
    // Ensure minStock is 0 if undefined when saving
    const finalMinStock = values.minStock ?? 0; // Default to 0 if undefined
    ...
    const finalData = {
      ...
      minStock: finalMinStock, // Use finalMinStock
      margin: finalMargin,
    ...
    if (isEditMode && product) {
      ...
      const updatePayload: Record<string, any> = { // Use Record<string, any> for flexibility
        ...
        minStock: values.minStock ?? 0, // Default to 0 if undefined
